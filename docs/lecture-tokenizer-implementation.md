...
| Токен | Назначение |
|-------|-----------|
| `<unk>` | Неизвестный токен (не используется в byte-level) |
...
**Причина:** Пары, встречающиеся только один раз в корпусе, не дают статистически значимой информации. Слияние таких пар раздувает словарь редкими бесполезными токенами. Для больших корпусов (50+ МБ) можно поднять до 3-5.

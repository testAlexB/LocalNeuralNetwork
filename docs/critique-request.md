# Запрос на peer review: Self-Attention + GQA для FishingLLM

**Контекст:** Мы (DeepSeek V4 Flash Free) пишем код для учебного проекта FishingLLM — decoder-only transformer ~150M параметров для генерации текста о рыбалке в Тверской области. AGENTS.md предписывает отправлять ключевые решения на ревью более сильным моделям перед переходом к следующему шагу.

**Прошу проверить:**

## 1. Реализация attention
Файл: `fishingllm/attention.py` (скопирован ниже)
...
1. **Корректность реализации:** Есть ли ошибки в тензорных операциях? Правильно ли работает GQA (repeat_interleave)? Не теряется ли градиентный поток?

2. **Эффективность:** 
   - Стоит ли заменить `repeat_interleave` на `expand` для экономии памяти?
   - Нужен ли `masked_fill` или лучше `softmax` с `-inf` через `torch.where`?
   - Эффективно ли объединение всех голов в batch через `reshape(-1, L, d_k)`?

3. **Численная стабильность:**
   - Корректен ли `masked_fill(mask, float("-inf"))` с точки зрения численной стабильности?
   - Нужен ли `torch.where(mask, float("-inf"), scores)` вместо `masked_fill`?

4. **Архитектурные решения:**
   - GQA 12:4 — разумный компромисс для 150M модели на CPU?
   - Стоит ли добавить Flash Attention в перспективе?
   - Правильно ли, что bias=False во всех проекциях?

5. **Тесты (12 тестов, все проходят):**
   - Достаточно ли тестов? Какие кейсы не покрыты?
   - Нужен ли тест на градиенты (backward pass)?
   - Есть ли смысл в тесте на численную эквивалентность с PyTorch-native F.scaled_dot_product_attention?

6. **Общие замечания:**
   - Какие антипаттерны вы видите?
   - Что бы вы сделали иначе?
   - Есть ли проблемы с масштабируемостью на контекст 2048+?

---

Буду благодарен за любой фидбек — от мелких придирок до архитектурных замечаний.
